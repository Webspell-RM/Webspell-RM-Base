...
<div class="panel panel-default">
            <div class="panel-heading">
                            <i class="fas fa-puzzle-piece"></i> Plugin Installer</div>
...
if(!empty($_GET['do'])) {
	$dir = $_GET['dir'];
	$dir = str_replace('/','',$dir);
	$plugin = 'http://t-seven.noip.me/plugin-base_v.2.0.1/';
...
//show all
try {
	$url = 'http://t-seven.noip.me/plugin-base_v.2.0.1/list.json';
	$result = curl_json2array($url);
	$anz = (count($result)-1);
	$output = "";
	$input = "";
	$install_datei ="";
	 ...
	 for($plug=1; $plug<=$anz; $plug++) {
        	...
        	$output .= '  <tr>';
		$output .= '<th><img src="http://t-seven.noip.me/plugin-base_v.2.0.1'.$result['item'.$plug]['path'].$result['item'.$plug]['preview'].'" class="img-plugin-picture" alt="{img}" /></th>';
		...
		include("../system/version.php");
			if(is_dir("../includes/plugins/".$result['item'.$plug]['path'])) {
				$output .= '<th>';$ergebnis = safe_query("SELECT * FROM `".PREFIX."plugins` WHERE `modulname`='".$result['item'.$plug]['modulname']."' LIMIT 1");
					...
					if($result['item'.$plug]['version_final']===$row['version']) { $output .='';  } else { $output .='<a class="btn btn-info" style="width: 150px" href="?site=plugin-installer&up=install&dir='.$result['item'.$plug]['path'].'">' . $_language->module['update'] . ' to Ver. '.$result['item'.$plug]['version_final'].'</a>';  }


$output .='<button class="btn btn-danger" style="width: 150px" data-href="?site=plugin-installer&deinstall=plugin&dir='.$result['item'.$plug]['path'].'" data-toggle="modal" data-target="#confirm-delete">
...
<i class="fas fa-puzzle-piece"></i> <?php echo $_language->module['all_plugins']; ?> &nbsp; (<?php echo $anz; ?>)</strong><br><br>
