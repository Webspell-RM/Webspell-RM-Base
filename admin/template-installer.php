...
<div class="panel panel-default">
            <div class="panel-heading">
                            <i class="fas fa-puzzle-piece"></i> Themes Installer</div>
		...
		include("../system/version.php");
			if(is_dir("../includes/themes/".$result['item'.$plug]['path'])) {
				$output .= '<th>';$ergebnis = safe_query("SELECT * FROM `".PREFIX."settings_themes` WHERE `modulname`='".$result['item'.$plug]['modulname']."' LIMIT 1");
					...
					if($result['item'.$plug]['version_final']===$row['version']) { $output .='';  } else { $output .='<a class="btn btn-info" style="width: 150px" href="?site=template-installer&up=install&dir='.$result['item'.$plug]['path'].'">' . $_language->module['update'] . ' to Ver. '.$result['item'.$plug]['version_final'].'</a>';  }


$output .='<button class="btn btn-danger" style="width: 150px" data-href="?site=template-installer&deinstall=plugin&dir='.$result['item'.$plug]['path'].'" data-toggle="modal" data-target="#confirm-delete">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
				...
				if($result['item'.$plug]['req']==$version) {
					$output .= '<th><a class="btn btn-success" style="width: 150px" href="?site=template-installer&do=install&dir='.$result['item'.$plug]['path'].'">' . $_language->module['installation'] . '</a></th>';
					$output .= '  </tr>';
				} else {
