...
if (isset($_GET[ 'delete' ])) {
    ...
    if ($CAPCLASS->checkCaptcha(0, $_GET[ 'captcha_hash' ])) {
        ...
        safe_query("DELETE FROM " . PREFIX . "settings_content_head_moduls WHERE modulID='" . $modulID . "' ");
    ...
    if ($CAPCLASS->checkCaptcha(0, $_POST[ 'captcha_hash' ])) {
        ...
        foreach ($sort as $sortstring) {
            ...
            safe_query("UPDATE " . PREFIX . "settings_content_head_moduls SET sort='".$sorter[1]."' WHERE modulID='".$sorter[0]."' ");
        }
    } else {
        echo $_language->module[ 'transaction_invalid' ];
    }
    ...
    if ($CAPCLASS->checkCaptcha(0, $_POST[ 'captcha_hash' ])) {
        ...
        $deactivated = 0;
		    ...
		    safe_query(
            "INSERT INTO
                `" . PREFIX . "settings_content_head_moduls` (
                    `module`,
                    `activated`,
                    `deactivated`,
                    `sort`
...
if ($action == "add") {
  ...
  <ul class="nav nav-tabs-primary">
    ...
    <li role="presentation" class="active"><a href="./admincenter.php?site=settings_content_head_moduls">Content Head Module</a></li>
...
<ol class="breadcrumb-primary"> </ol><br>
	...
	echo'<form class="form-horizontal" method="post" action="admincenter.php?site=settings_content_head_moduls" enctype="multipart/form-data">
...
<img class="img-thumbnail" src="../images/plugins/layout-head-content.jpg">
</div>
<div class="col-sm-2">
    <label for="activated">'.$_language->module['deactivated'].'</label>
  <input id="deactivated" type="radio" name="radio1" value="activated">
<img class="img-thumbnail" src="../images/plugins/layout-head-content-ohne.jpg">
</div>
<label class="col-sm-2 control-label"></label>
</div>
...
<ol class="breadcrumb-primary"> </ol><br>
  ...
  $modulID = $_GET[ 'modulID' ];
    $ergebnis = safe_query("SELECT * FROM " . PREFIX . "settings_content_head_moduls WHERE modulID='$modulID'");
...
';
  ...
  echo'<a href="admincenter.php?site=settings_content_head_moduls&amp;action=add" class="btn btn-primary" type="button">' . $_language->module[ 'new_modul' ] . '</a><br /><br />';

	echo'<form method="post" action="admincenter.php?site=settings_content_head_moduls">
  <table class="table table-striped">
    <thead>
      <th><b>'.$_language->module['module'].'</b></th>
      <th class="hidden-sm hidden-xs"><img style="width: 250px;" class="img-thumbnail" src="../images/plugins/layout-head-content.jpg"><br><b>'.$_language->module['activated'].'</b></th>
      
      <th class="hidden-sm hidden-xs"><img style="width: 250px;" class="img-thumbnail" src="../images/plugins/layout-head-content-ohne.jpg"><br><b>'.$_language->module['deactivated'].'</b></th>
      <th><b>'.$_language->module['actions'].'</b></th>
      <th><b>'.$_language->module['sort'].'</b></th>
    </thead>';

	$moduls = safe_query("SELECT * FROM " . PREFIX . "settings_content_head_moduls ORDER BY sort");
    $tmp = mysqli_fetch_assoc(safe_query("SELECT count(modulID) as cnt FROM " . PREFIX . "settings_content_head_moduls"));
    $anzmoduls = $tmp[ 'cnt' ];
    $CAPCLASS = new \webspell\Captcha;
    $CAPCLASS->createTransaction();
    $hash = $CAPCLASS->getHash();
    ...
    while ($db = mysqli_fetch_array($moduls)) {
      ...
      <td><a href="admincenter.php?site=settings_content_head_moduls&amp;action=edit&amp;modulID='.$db['modulID'].'" class="hidden-xs hidden-sm btn btn-warning" type="button">' . $_language->module[ 'edit' ] . '</a>

        <input class="hidden-xs hidden-sm btn btn-danger" type="button" onclick="MM_confirm(\'' . $_language->module['really_delete'] . '\', \'admincenter.php?site=settings_content_head_moduls&amp;delete=true&amp;modulID='.$db['modulID'].'&amp;captcha_hash='.$hash.'\')" value="' . $_language->module['delete'] . '" />

	  <a href="admincenter.php?site=settings_content_head_moduls&amp;action=edit&amp;modulID='.$db['modulID'].'"  class="mobile visible-xs visible-sm" type="button"><i class="fa fa-pencil"></i></a>
      <a class="mobile visible-xs visible-sm" type="button" onclick="MM_confirm(\'' . $_language->module['really_delete'] . '\', \'admincenter.php?site=settings_content_head_moduls&amp;delete=true&amp;modulID='.$db['modulID'].'&amp;captcha_hash='.$hash.'\')" /><i class="fa fa-times"></i></a>
