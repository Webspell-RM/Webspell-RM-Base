...
if ($action == "add") {
  ...
  <ul class="nav nav-tabs-primary">
    ...
    <li role="presentation" class="active"><a href="./admincenter.php?site=settings_head_moduls">Page Head Module</a></li>
...
';
  ...
  echo'<a href="admincenter.php?site=settings_head_moduls&amp;action=add" class="btn btn-primary" type="button">' . $_language->module[ 'new_modul' ] . '</a><br /><br />';
	...
	echo'<form method="post" action="admincenter.php?site=settings_head_moduls">
  <table class="table table-striped">
	...
	$moduls = safe_query("SELECT * FROM " . PREFIX . "settings_head_moduls ORDER BY sort");
    $tmp = mysqli_fetch_assoc(safe_query("SELECT count(modulID) as cnt FROM " . PREFIX . "settings_head_moduls"));
    $anzmoduls = $tmp[ 'cnt' ];
    $CAPCLASS = new \webspell\Captcha;
    $CAPCLASS->createTransaction();
    $hash = $CAPCLASS->getHash();
